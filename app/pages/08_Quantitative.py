...
if not cmd.empty:
    cols = [c for c in ["asset", "spot_price", "daily_change", "z_score", "rsi", "macd",
                        "hist_vol", "volatility_regime", "curve_structure", "momentum"] if c in cmd.columns]
    st.dataframe(cmd[cols], use_container_width=True)

    # Correlation requires multi-day history; with single-day mock it will be empty/partial
    corr = correlation_matrix(cmd)
    if not corr.empty:
        st.plotly_chart(correlation_heatmap(corr), use_container_width=True)
    else:
        st.info("Correlation matrix requires multi-period history (will populate after historical accumulation).")
else:
    st.warning("No data.")
