...
if hist.empty:
    st.info("Historical parquet not yet accumulated. Single-day mock is shown below. History will grow with daily RAW ingestion (Phase 2).")
    if not cmd.empty:
        st.dataframe(cmd, use_container_width=True)
else:
    window = st.selectbox("Window", [7, 30, 90, 365, None], format_func=lambda x: "Full" if x is None else f"{x}d")
