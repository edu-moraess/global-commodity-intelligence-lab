...
st.set_page_config(page_title="Agriculture | GCIL", layout="wide")
inject_css()
st.title("🌾 Agriculture")

repo = get_repository()
cmd, _, _, meta = repo.load_latest()
df = cmd[cmd["category"] == "AGRICULTURE"] if not cmd.empty else cmd
n = len(df)
q = quality_report(df).get("completeness") if not df.empty else None
status_bar(meta, n_records=n, quality=q)

if not df.empty:
    st.plotly_chart(price_bar(df, "Agriculture Spot Prices"), use_container_width=True)
    st.plotly_chart(ranking_bar(df), use_container_width=True)
    st.dataframe(df, use_container_width=True)
else:
    st.info("No agriculture records.")
