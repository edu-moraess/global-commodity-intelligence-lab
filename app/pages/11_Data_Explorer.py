...
if cmd.empty:
    ...
    assets = (
        sorted(cmd["asset"].unique().tolist())
        if cat == "ALL"
        else sorted(cmd[cmd["category"] == cat]["asset"].unique().tolist())
    )
    asset = st.multiselect("Asset(s)", assets, default=assets[:5] if assets else [])
    metrics = [
        c for c in cmd.columns
        if c not in ("date", "asset", "category", "ticker", "source")
    ]
    ...
    cols = ["date", "asset", "category", metric] if metric in view.columns else list(view.columns)
